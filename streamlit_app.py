...
def main():
    ...
    if recipe_to_cook is not None:
            # Search for YouTube videos with English transcripts
        eng_recipes = get_eng_recipe(recipe_to_cook)

        # Let the user select a video
        video_titles = [recipe['title'] for recipe in eng_recipes]
        index_of_recipe_chosen = st.selectbox("Select a video", options=range(
            len(video_titles)), format_func=lambda x: video_titles[x])
    ...
    if loaded_data is not None and len(loaded_data) > 0 and recipe_chosen['id'] in loaded_data[0].keys():
        dictionary = loaded_data[0][recipe_chosen['id']]
        print("laoding from cache")
        ...
        print("not in cache")
        transcript = get_transcript(recipe_chosen['id'])
        transcript_list = [i['text'] for i in transcript]
        transcript_combined = "".join(f"{i} " for i in transcript_list)
        model_role, prompt = create_prompt_for_recipe(
            transcript, recipe_to_cook)
        gemini_output = get_gemini_response(model_role, prompt)
        steps, ingredients, servings, utensils = extract_recipe_details(
            gemini_output)
        if steps is None:
            gemini_output = get_gemini_response(
                model_role, prompt, temperature=1)
            steps, ingredients, servings, utensils = extract_recipe_details(
                gemini_output)
        model_role_time, time_stamp_prompt = create_prompt_for_timestamps(
            transcript, steps, recipe_to_cook)
        time_gemini_output = get_gemini_response(
            model_role_time, time_stamp_prompt)
        timestamps = extract_timestamps(time_gemini_output)
        if timestamps is None:
            time_gemini_output = get_gemini_response(
                model_role_time, time_stamp_prompt, temperature=1)
            timestamps = extract_timestamps(time_gemini_output)
        base_youtube_video_url = f"https://www.youtube.com/embed/{recipe_chosen['id']}"
        timestamps_urls = {
    ...
    # Display recipe details
        ...
        if st.session_state['selected_timestamp_url'] is not None:
            youtube_video_url = st.session_state['selected_timestamp_url']
            st.components.v1.html(
                f'<iframe width="560" height="315" src="{youtube_video_url}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>',
                height=315,)
        else:
